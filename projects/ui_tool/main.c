...
// Smoke output goes beside the build when it is there, and into the working directory otherwise,
// so a run started from anywhere still has somewhere to write.
static const char *Scratch(const char *name)
{
    static char path[256];
    if (DirectoryExists("build/core"))
        snprintf(path, sizeof path, "build/core/%s", name);
    else
...
static void SmokeChecks(Tool *tool)
{
    ...
    bool saved = UiDocumentSave(&round, Scratch("ui_tool_roundtrip.ui"));
    UiDocument reloaded = {0};
    bool loaded = saved && UiDocumentLoad(&reloaded, Scratch("ui_tool_roundtrip.ui"));
    Check(tool,
    ...
    const char *legacyPath = Scratch("ui_tool_v1.ui");
    FILE *legacy = fopen(legacyPath, "wb");
    if (legacy)
    {
        fprintf(legacy, "core_ui_document 1\n0 32 32 140 26 0.5 0\tButton\n");
        fclose(legacy);
        UiDocument old = {0};
        bool ok = UiDocumentLoad(&old, legacyPath);
        Check(tool, ok && old.count == 1 && old.surfaceWidth > 0,
              "version 1 layouts still load and receive a default surface");
        UiDocumentFree(&old);
    }

    // Path resolution, which only shows itself when the program is started from somewhere else.
    {
        char buf[256];
        const char *root = "build/core/ui_tool_root";
        char nested[320];
        snprintf(nested, sizeof nested, "%s/core/shaders", root);
        char deep[320];
        snprintf(deep, sizeof deep, "%s/build/core", root);
        MakeDirectory(nested);
        MakeDirectory(deep);
        char probe[384];
        snprintf(probe, sizeof probe, "%s/probe.fs", nested);
        SaveFileText(probe, "// probe\n");

        // Given the binary's own directory, the root walks up until the engine data is in reach.
        CoreSetDataRoot(deep);
        char *found = CoreReadFile("core/shaders/probe.fs");
        Check(tool, found != NULL,
              "the data root walks up from the executable until it finds the engine's files");
        CoreFreeFile(found);

        Check(tool, !strcmp(CoreResolvePath("/etc/hostname", buf, sizeof buf), "/etc/hostname"),
              "an absolute path is left exactly as it was given");
        char local[256];
        snprintf(local, sizeof local, "%s", Scratch("ui_tool_local.fs"));
        SaveFileText(local, "// local\n");
        Check(tool, !strcmp(CoreResolvePath(local, buf, sizeof buf), local),
              "a file that exists from the working directory still wins over the root");
        // "core" is a directory here, so an existence test that accepted directories would keep it.
        const char *shadowed = CoreResolvePath("core", buf, sizeof buf);
        Check(tool, shadowed && strcmp(shadowed, "core") != 0,
              "a directory of the same name does not shadow the file being looked for");
        char tiny[8];
        Check(tool, CoreResolvePath("core/shaders/probe.fs", tiny, sizeof tiny) == NULL,
              "a path that will not fit is reported as a miss, not silently shortened");

        // Put back the root EngineRun chose, or later glyph loads would look in the wrong place.
        CoreSetDataRoot(GetApplicationDirectory());
    }

    // Capabilities: core must enforce what the project declared, and nothing else.
    {
        char home[1024];
        const char *back = getcwd(home, sizeof home) ? home : NULL;
        // Somewhere with no engine data, so an undeclared shader cannot be quietly found anyway.
        if (back && chdir("/") == 0)
        {
            CoreSetDataRoot("/");
            Check(tool, CoreCheckCapabilities((CoreRequirements){0}),
                  "a project that declares nothing boots without probing or compiling anything");
            Check(tool, !CoreCheckCapabilities((CoreRequirements){.gpuSkinning = true}),
                  "declaring GPU skinning is what pulls core's skinning shader in");
            CoreSetDataRoot(GetApplicationDirectory());
            if (chdir(back) != 0)
                tool->failures++;
        }
        Check(tool, !CoreCheckCapabilities((CoreRequirements){.textureUnits = 1 << 20}),
              "a declared limit the card cannot meet is fatal, with no fallback");
        Check(tool, CoreCheckCapabilities((CoreRequirements){.renderTargets = true}),
              "a declared capability the card does have passes");
    }
}
...
static void SmokeFrame(Tool *tool)
{
    ...
    *input = (EngineInput){0};
    ...
    // Whatever size the window manager handed us, click inside the workspace: the middle of where
    // it overlaps the surface when it does, and its own middle when the surface is off to one side.
    UiRect shown = ToScreen(tool, (UiRect){0, 0, tool->document.surfaceWidth,
                                           tool->document.surfaceHeight});
    int left = MaxInt(shown.x, tool->workspace.x);
    int top = MaxInt(shown.y, tool->workspace.y);
    int right = MinInt(shown.x + shown.width, tool->workspace.x + tool->workspace.width);
    int bottom = MinInt(shown.y + shown.height, tool->workspace.y + tool->workspace.height);
    Vector2 surfacePoint = {(float)(tool->workspace.x + tool->workspace.width / 2),
                            (float)(tool->workspace.y + tool->workspace.height / 2)};
    if (right - left > 24 && bottom - top > 24)
        surfacePoint = (Vector2){(float)((left + right) / 2), (float)((top + bottom) / 2)};
    ...
    else if (frame >= 10 && frame <= 16)
    {
        ...
        else
        {
            ...
            // Large enough to close the distance whatever size the workspace turned out to be;
            // the step shrinks as it arrives, so it cannot overshoot the snap.
            input->mousePosition.x += (float)ClampInt(stepX, -64, 64);
            input->mousePosition.y += (float)ClampInt(stepY, -64, 64);
        }
    }
    else if (frame == 17)
    {
    ...
    else if (frame == 30)
    {
        ...
        // Both the button and the well below it have to fit on the surface, wherever the click was.
        docX = ClampInt(docX, 0, MaxInt(0, tool->document.surfaceWidth - 100));
        docY = ClampInt(docY, 0, MaxInt(0, tool->document.surfaceHeight - 40 - 60));
    ...
    else if (frame == 60)
    {
        ...
        // Ordinary text must never cost an atlas rebuild: the id only changes when one happens.
        tool->atlas = tool->ui.theme.font.texture.id;
        snprintf(tool->document.elements[2].label, sizeof tool->document.elements[2].label,
                 "Ordinary ASCII 0123 !?");
    }
    else if (frame == 61)
    {
        Check(tool, tool->ui.theme.font.texture.id == tool->atlas,
              "printable ASCII is baked in, so ordinary text never rebuilds the atlas");
        snprintf(tool->document.elements[2].label, sizeof tool->document.elements[2].label,
                 "\xd7\x90");
    }
    else if (frame == 62)
        Check(tool, tool->ui.theme.font.texture.id != tool->atlas,
              "a character outside the baked range is still loaded the first time it is seen");
    else if (frame == 41)
    {
        UiElement *element = &tool->document.elements[2];
        UiRect inner = ContainerContent(tool, &tool->document.elements[3]);
...
static bool Update(void *context, double dt, const EngineInput *input)
{
    ...
    Tool *tool = context;
    if (tool->smoke)
        return tool->frames < 64 && !tool->failures;
...
static void Draw(void *context, float alpha)
{
    ...
    if (tool->smoke && tool->frames == 55)
    {
        ...
        ExportImage(shot, Scratch("ui_tool_text.png"));
        UnloadImage(shot);
    }
    if (tool->smoke &&
         ...
         tool->frames == 47))
    {
        ...
        Image shot = LoadImageFromScreen();
        ExportImage(shot, Scratch(tool->frames == 14   ? "ui_tool_drag.png"
                                  : tool->frames == 19 ? "ui_tool.png"
                                  : tool->frames == 23 ? "ui_tool_preview.png"
                                  : tool->frames == 25 ? "ui_tool_screen.png"
                                                       : "ui_tool_resize.png"));
        UnloadImage(shot);
    }
    tool->frames++;
...
int main(int argc, char **argv)
{
    ...
    EngineConfig config = {.title = "Core UI tool",
                           .width = 1100,
                           .height = 700,
                           .targetFps = 60,
                           .fixed_dt = 0,
                           ...
                           /* The workspace draws the surface into a texture; nothing else. */
                           .requirements = {.renderTargets = true}};
